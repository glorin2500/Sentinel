                                    ...
                                    <div className="flex-1 p-8 flex flex-col md:flex-row items-center gap-8">
                                        ...
                                        <div className="flex-1 space-y-4 text-center md:text-left">
                                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                                ...
                                                <div className={`text-[10px] font-black px-4 py-2 rounded-xl uppercase tracking-[0.2em] border-2 self-center md:self-auto ${scan.status === 'safe'
                                                    ? 'bg-primary/20 border-primary/20 text-primary'
                                                    : 'bg-destructive/20 border-destructive/20 text-destructive shadow-[0_4px_20px_rgba(255,107,107,0.2)]'
                                                    }`}>
                                                    {scan.status === 'safe' ? 'IDENTITY_SECURE' : (scan.threatType || 'THREAT_LOGGED')}
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-white/5">
                                                <div className="space-y-1">
                                                    ...
                                                    <p className="text-[10px] font-bold text-zinc-300 uppercase italic">SHA-512_SECURE</p>
                                                </div>
                                                <div className="space-y-1">
                                                    <p className="text-[9px] font-black text-zinc-600 uppercase tracking-widest">Zone</p>
                                                    <p className="text-[10px] font-bold text-zinc-300 flex items-center justify-center md:justify-start gap-1">
                                                ...
                                                <div className="space-y-1">
                                                    ...
                                                    <p className="text-[10px] font-bold text-zinc-300 flex items-center justify-center md:justify-start gap-1 font-mono uppercase">
                                                        {scan.id.substring(0, 8)}
                                                    </p>
                                                </div>
                                                <div className="space-y-1">
                                                    <p className="text-[9px] font-black text-zinc-600 uppercase tracking-widest">Integrity</p>
                                                    <p className={`text-[10px] font-bold ${scan.status === 'safe' ? 'text-primary' : 'text-destructive'}`}>
                                                        {scan.status === 'safe' ? 'NOMINAL' : 'COMPROMISED'}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>

                                        <div className="shrink-0 flex items-center justify-center md:border-l border-white/5 md:pl-8">
                                            <button className="p-4 rounded-2xl bg-white/5 border border-white/5 text-zinc-500 hover:text-white hover:border-white/20 hover:bg-white/10 transition-all group-hover:translate-x-1">
